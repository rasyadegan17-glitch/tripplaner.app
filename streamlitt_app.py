...
st.title("✈️ Trip Planner Sat-Set (Mode 1 Halaman)")
...
with tab2:
    ...
    if not st.session_state.trips:
        st.warning("⚠️ Belum ada Trip yang dibuat. Silakan ke Tab 'Buat Trip'.");
    else:
        # Pilihan Trip
...
with tab4:
    ...
    for trip in st.session_state.trips:
        ...
        with st.expander(f"🗺️ **{trip.title}** (Total Biaya: Rp {trip.total_cost():,.0f})"):
